...
import { Package, Heart, MapPin, Wallet, Tag, Bell, Settings, LogOut, ChevronRight, User, Ticket, Store } from 'lucide-react';
import { BottomNav } from '../components/BottomNav';
import { Header } from '../components/Header';
import { useAuthStore } from '../store/useAuthStore';

export function ProfilePage() {
  const { user, token, logout, selectedStore, clearStore } = useAuthStore();
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col pb-20">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 mt-8">
          ...
          <Link to="/login" className="bg-brand-blue text-white font-bold px-8 py-3 rounded-xl text-sm">Login</Link>
          <Link to="/signup" className="text-brand-blue text-sm font-semibold">Create Account</Link>
        </div>
        <BottomNav />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <Header title="My Profile" />
      <div className="bg-brand-blue text-white px-6 pt-6 pb-8 rounded-b-[2.5rem] shadow-sm">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-bold">{selectedStore ? selectedStore.name : user?.name}</h1>
            <p className="text-xs text-white/80 mt-0.5">{selectedStore ? (selectedStore.owner_name || selectedStore.phone || selectedStore.email) : (user?.phone || user?.email)}</p>
            <Link to="/dashboard"
      ...
      <div className="p-4 mt-2 max-w-4xl mx-auto">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {selectedStore && (
            <button onClick={() => { clearStore(); navigate('/select-store'); }}
              ...
              <div className="flex items-center gap-3 text-orange-500">
                ...
                <span className="text-xs font-semibold">Change Store</span>
              </div>
              <ChevronRight className="w-4 h-4 text-gray-400" />
            </button>
          )}
