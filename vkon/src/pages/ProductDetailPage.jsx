...
export function ProductDetailPage() {
  ...
  if (!product) return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-brand-beige text-gray-900 gap-4">
      <h2 className="text-2xl font-bold font-serif">Product Not Found</h2>
      <button onClick={() => navigate('/')} className="bg-brand-blue text-white px-8 py-3 rounded-2xl font-bold">Back to Home</button>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-white font-sans pb-28">
      ...
      <div className="md:hidden">
        {/* Hero Background */}
        <div className="bg-gradient-to-b from-brand-beige to-white relative pt-4 pb-10">
          ...
          <div className="flex items-center justify-center px-5 mt-6 relative z-0">
            <button
              onClick={() => setLightboxOpen(true)}
              className="relative group focus:outline-none bg-white w-full max-w-[340px] aspect-[4/3] rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-brand-blue/5 flex items-center justify-center transition-transform active:scale-95"
            >
              <motion.img
                key={mainImg}
              ...
              />
              {/* Zoom hint overlay */}
              <span className="absolute bottom-3 right-3 bg-white/90 backdrop-blur rounded-full p-2.5 shadow-sm text-gray-500 hover:text-brand-blue transition-colors">
        ...
        <div className="bg-white rounded-t-[32px] -mt-6 relative z-10 px-5 pt-6">
                  ...
                  return (
                    <button key={i} onClick={() => handleColorChange(v.color)}
                      className={`px-4 py-2 rounded-xl border-2 text-sm font-bold transition-all ${
                        isSelected
                          ? 'border-[#0033a0] text-[#0033a0] bg-red-50'
                  ...
                  return (
                    <button key={i} onClick={() => setSelectedSize(s)}
                      className={`px-5 py-2.5 rounded-full border-2 text-sm font-bold transition-all ${
                        isSelected
                          ? 'bg-[#0033a0] border-[#0033a0] text-white shadow-md scale-105'
          ...
          <div className="grid grid-cols-2 gap-2 mt-6">
            {[
              { icon: <Truck className="w-4 h-4 text-[#0033a0]" />, label: 'Fast Delivery' },
              { icon: <RotateCcw className="w-4 h-4 text-[#0033a0]" />, label: 'Easy Returns' },
              { icon: <Check className="w-4 h-4 text-[#0033a0]" />, label: '100% Quality' },
              { icon: <Star className="w-4 h-4 text-brand-orange fill-brand-orange" />, label: 'Top Rated' },
            ].map(({ icon, label }) => (
                ...
                {relatedProducts.map(p => {
                  ...
                  return (
                    <button key={p.id} onClick={() => navigate(`/product/${p.id}`)} className="flex-shrink-0 w-28 group">
                      <div className="w-28 h-28 bg-brand-beige rounded-2xl overflow-hidden border border-brand-blue/10 group-hover:scale-105 transition-transform">
            ...
            <div className="mt-6 bg-gray-50 rounded-2xl p-4 mb-4">
              ...
              {reviewSuccess ? (
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {[1,2,3,4,5].map(star => (
                      <button type="button" key={star} onClick={() => setReviewForm({ ...reviewForm, rating: star })}>
                        <Star className={`w-5 h-5 ${star <= reviewForm.rating ? 'fill-brand-orange text-brand-orange' : 'text-gray-200'}`} />
                      </button>
                    ))}
                  </div>
                  ...
                  <button type="submit" disabled={reviewSubmitting} className="w-full py-3 bg-[#0033a0] text-white font-bold rounded-xl text-sm disabled:opacity-60">
      ...
      <div className="hidden md:block bg-gray-50 min-h-screen pt-4 pb-20">
        <div className="max-w-6xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-500 py-4">
            <button onClick={() => navigate('/')} className="hover:text-[#0033a0] transition-colors">Home</button>
            <span>/</span>
            <button onClick={() => navigate(`/category/${product.category}`)} className="capitalize hover:text-[#0033a0] transition-colors">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 lg:p-10 mb-8 grid grid-cols-12 gap-10">
            ...
            <div className="col-span-5 flex flex-col items-center">
              <div className="relative w-full h-[380px] bg-brand-beige/50 rounded-2xl p-6 flex items-center justify-center border border-brand-blue/10 group">
                <img
                ...
                >
                  <ZoomIn className="w-4 h-4 text-[#0033a0]" /> Zoom
                </button>
              </div>
                  ...
                  {images.map((img, i) => (
                    <button
                      ...
                      className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all shrink-0 ${
                        mainImg === img ? 'border-[#0033a0] shadow-md scale-105' : 'border-gray-200 opacity-70 hover:opacity-100'
                      }`}
                    >
                      <img src={img} alt="" className="w-full h-full object-cover" />
                    </button>
            ...
            <div className="col-span-7 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-[#0033a0] bg-red-50 px-3 py-1 rounded-full uppercase tracking-wider">
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-2.5 rounded-xl border-2 text-sm font-bold transition-all ${
                              isSelected
                                ? 'border-[#0033a0] text-[#0033a0] bg-red-50 shadow-sm'
                      ...
                      {sizes.map((s, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-2.5 rounded-xl border-2 text-sm font-bold transition-all ${
                              isSelected
                                ? 'bg-[#0033a0] border-[#0033a0] text-white shadow-md'
                                : 'border-gray-200 text-gray-700 bg-white hover:border-gray-300'
                            }`}
                          >
                            {s.size}
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2.5 py-3.5 rounded-2xl font-bold text-base transition-all ${
                      isOutOfStock
                        ...
                        : 'bg-[#0033a0] text-white hover:bg-[#b81633] shadow-lg shadow-[#0033a0]/20 active:scale-[0.98]'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-8 py-3.5 rounded-2xl font-bold text-base border-2 transition-all ${
                      isOutOfStock
                        ...
                        : 'border-[#0033a0] text-[#0033a0] hover:bg-red-50 active:scale-[0.98]'
                    }`}
                  >
                    Buy Now
                  </button>
                    ...
                    <form onSubmit={handleReviewSubmit} className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <input
                          ...
                          className="px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-[#0033a0] bg-white"
                        />
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-[#0033a0] bg-white resize-none"
                      />
                      <button
                        type="submit"
                        disabled={reviewSubmitting}
                        className="px-6 py-2.5 bg-[#0033a0] text-white font-bold rounded-xl text-sm hover:bg-[#b81633] transition-colors disabled:opacity-60"
                      >
                        {reviewSubmitting ? 'Submitting...' : 'Submit Review'}
                      </button>
                    </form>
      ...
      <div className="fixed bottom-0 left-0 w-full z-[60] md:hidden">
        <div className="bg-white border-t border-gray-100 px-4 py-3 flex items-center gap-3 shadow-2xl">
          ...
          <button onClick={handleAddToCart} disabled={isOutOfStock}
            className={`flex-1 flex items-center justify-center gap-2 font-bold py-4 rounded-2xl text-base transition-all active:scale-95 ${isOutOfStock ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : addedFeedback ? 'bg-green-500 text-white' : 'bg-[#0033a0] text-white shadow-lg shadow-[#0033a0]/30'}`}>
            <ShoppingCart className="w-5 h-5" />
            {isOutOfStock ? 'Out of Stock' : addedFeedback ? '✓ Added to Cart!' : 'Add to Cart'}
          </button>
        </div>
