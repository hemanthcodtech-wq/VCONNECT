          ...
          {suggestions.map(s => (
            <li key={s.place_id}>
              <button type="button" onMouseDown={(e) => { e.preventDefault(); handleSelect(s); }}
                className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-brand-blue text-white/5 flex items-start gap-2.5 border-b border-gray-50 last:border-0">
      ...
      <div className="bg-white rounded-t-3xl md:rounded-2xl w-full max-w-lg p-6 max-h-[92vh] overflow-y-auto">
        ...
        <label className="flex items-center gap-2.5 mt-4 cursor-pointer">
          <div onClick={() => setForm(f => ({ ...f, is_default: !f.is_default }))}
            className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${form.is_default ? 'bg-brand-blue text-white border-brand-blue/10' : 'border-gray-300'}`}>
        ...
        <button onClick={handleSubmit} disabled={saving}
          className="w-full mt-5 bg-brand-blue text-white font-bold py-3.5 rounded-xl text-sm hover:bg-gray-600 transition-colors disabled:opacity-60">
          ...
          {selectedStore ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <button onClick={() => setShowForm(true)}
                className="bg-brand-blue text-white text-sm font-bold px-6 py-2.5 rounded-xl hover:bg-gray-600 transition-colors">
                Add Address
              </button>
            </div>
          ) : (
