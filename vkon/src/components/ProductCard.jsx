...
export function ProductCard({ product, layout = 'grid', searchQuery = '' }) {
  ...
  const iconButtonClass =
    'flex h-8 w-8 items-center justify-center rounded-full bg-white/90 text-brand-maroon/60 shadow-sm transition-transform hover:scale-105 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-brand-gold motion-reduce:transition-none';
    ...
    return (
      ...
      >
        <div className="relative h-28 w-28 shrink-0 overflow-hidden rounded-[18px] bg-brand-cream/60 p-2">
          {activeOffer && (
            <span className="absolute left-2 top-2 z-10 rounded-full bg-brand-blue px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-white">
        ...
        <div className="absolute right-3 top-3 flex gap-1">
          ...
          >
            <Heart className={`h-4 w-4 ${isWishlisted ? 'fill-brand-blue text-brand-blue' : ''}`} />
          </button>
          <button
            type="button"
        ...
        {isOutOfStock ? (
          <span className="absolute bottom-4 right-4 rounded-full border border-brand-blue/20 bg-red-50 px-3 py-1.5 text-xs font-semibold text-brand-blue">
            Out of stock
          </span>
        ) : (
          <button
  ...
  return (
    ...
    >
      ...
      <div className="relative -mx-2.5 -mt-2.5 mb-3 sm:-mx-3 sm:-mt-3 aspect-square overflow-hidden rounded-t-[23px] bg-[#FDF8F0]">
        {firstImg ? (
          <img
            src={firstImg}
            alt={imageAlt}
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="mt-auto flex flex-col gap-3 pt-3">
            ...
            <div className="flex flex-wrap items-baseline gap-1.5 mt-0.5">
              ...
              {(activeOffer || originalPrice > displayPrice) && (
                <>
                  ...
                  {activeOffer && (
                    <span className="text-[9px] font-bold text-[#0033a0]">{parseFloat(activeOffer.discount_percentage)}% OFF</span>
                  )}
                </>
              )}
            </div>
          ...
          {isOutOfStock ? (
            <span className="w-full text-center rounded-xl border border-brand-blue/20 bg-red-50 px-3 py-2 text-[12px] font-semibold text-brand-blue">
              Out of stock
            </span>
          ) : (
            <button
              ...
              title="Add to cart"
              onClick={handleAddToCart}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#0033a0] to-[#15803d] py-2 text-sm font-semibold text-white shadow-md transition-all hover:scale-[1.02] hover:shadow-lg active:scale-95"
            >
              <ShoppingCart className="h-4 w-4" />
